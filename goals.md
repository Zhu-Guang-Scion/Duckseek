...
- 【阶段二】2026-09-22 O2 自验观察：CLI `ask` 未捕获 generate 期 LlmError——认证失败等场景以完整 traceback 呈现而非友好错误行（HTTP 401 实测）；应在 cli.py ask 路径映射 LlmError 为红色单行提示。另：本会话持有的 LLM 密钥已于 09-20 轮换作废（401），新密钥待人类提供。
- 【公开后完善清单】2026-09-23 开源准备完成后的后续：①英文 README（中文先行既定策略）；②包级重命名 nl2data → duckseek（CLI 同步更名 + 迁移说明，阶段三）；③CONTRIBUTING 由 docs/architecture.md §6 开发指南蒸馏。提示词方案文档不入库的声明已兑现——仓库文件树零「方案/提示词」命名文件（git ls-files 核验）。
## §8 变更规则
